...
int main() {
  ...
  // matrix of layouts
  matrix<mplr::subarray_layout<int>> sub_matrix_l(px, py);
  ...
  for (int iy{0}; iy < py; ++iy) {
    for (int ix{0}; ix < px; ++ix) {
      ...
      nx_0(ix, iy) = nx * ix / px;
      ny_0(ix, iy) = ny * iy / py;
      sub_matrix_l(ix, iy) = mplr::subarray_layout<int>(
  ...
  // gather via send-recv
  {
    ...
    mplr::contiguous_layout<int> matrix_l(nx_l(px_l, py_l) * ny_l(px_l, py_l));
    // send local sub-matrix to rank 0
    mplr::irequest r(comm_world.isend(m_l.data(), matrix_l, 0));
    if (p_l == 0) {
  ...
  // gather via alltoallv
  {
    ...
    const int root{0};
    mplr::layouts<int> sendl, recvl;
    for (int i{0}; i < p; ++i) {
      if (i == root)
        sendl.push_back(mplr::contiguous_layout<int>(nx_l(px_l, py_l) * ny_l(px_l, py_l)));
      else
        sendl.push_back(mplr::empty_layout<int>());
    }
    if (p_l == root) {
      for (int i{0}; i < p; ++i)
        recvl.push_back(sub_matrix_l(i % px, i / px));
      ...
      for (int i{0}; i < p; ++i)
        recvl.push_back(mplr::empty_layout<int>());
