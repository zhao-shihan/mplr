...
int main() {
  ...
  if (comm_world.rank() == 1) {
    std::list<int> v(20, 0);
    mplr::iterator_layout<int> l(v.begin(), v.end());
  ...
  if (comm_world.rank() == 0) {
    ...
    mplr::vector_layout<double> lv(v.size());
    mplr::heterogeneous_layout l(
        y, pair, mplr::make_absolute(v.data(), lv));  // heterogeneous_layout with 3 elements
    comm_world.send(mplr::absolute, l, 1);            // send data to rank 1
  }
  if (comm_world.rank() == 1) {
    double y{0};
    ...
    mplr::heterogeneous_layout l(
        y, pair, mplr::make_absolute(v.data(), lv));  // heterogeneous_layout with 3 elements
    comm_world.recv(mplr::absolute, l, 0);            // receive data from rank 0
