  ...
  if (comm_world.rank() == 0) {
    ...
    }
    {
      ...
      while (true) {
        ...
        if (finished.first ==
            mplr::test_result::no_active_requests)  // there have been no pending sends
          break;
        // print indices of finished sends
        std::cout << "send finished : ";
        std::for_each(finished.second.begin(), finished.second.end(),
                      [](mplr::irequest_pool::size_type j) { std::cout << j << ' '; });
        std::cout << "\n";
      }
    }
  }
  ...
  if (comm_world.rank() == 1) {
    ...
    }
    {
      ...
      while (true) {
        ...
        if (finished.first ==
            mplr::test_result::no_active_requests)  // there have been no pending receives
          break;
